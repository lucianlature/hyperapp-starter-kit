...
export default function createLogicAction$({
  action,
  logic,
  state,
  deps,
  ...
  const logicAction$ = Observable.create((logicActionObs) => {
    ...
    function handleNextOrDispatch(shouldProcess, act, options) {
      ...
      if (shouldDispatch(act, useDispatch)) {
        monitor$.next({
          action, dispAction: act, name, shouldProcess, op: 'nextDisp',
        });
        interceptComplete = true;
        ...
        if (act) {
          monitor$.next({
            action, nextAction: act, name, shouldProcess, op: 'next',
          });
        } else { // act is undefined, filtered
          monitor$.next({
            action, name, shouldProcess, op: 'filtered',
          });
          interceptComplete = true;
        }
